...
public class PriorityScheduler extends Scheduler {
	...
	protected class PriorityQueue extends ThreadQueue {
		PriorityQueue(boolean transferPriority) {
			...
			waitPQueue = new HashSet<ThreadState>();
			resourceOwner = null;
		}
		...
		public KThread nextThread() {
			...
			// implement me
			//print();
			ThreadState peek = pickNextThread();
			if (peek != null){
				//remove from waitQueue for the resource
				waitPQueue.remove(peek);
				//this thread will acquire the resource that the queue holds
				peek.acquire(this);
				//return the thread
				return peek.thread;
			}
			//if pickNextThread returns null, return null as well
			return null;
		}
		...
		protected ThreadState pickNextThread() {
			Iterator next = waitPQueue.iterator();
			Iterator next1 = waitPQueue.iterator();
			int maxPriority = -1;
			ThreadState maxThread = null;
			while(next.hasNext()){
				ThreadState nextThreadState = (ThreadState) next.next();
				if(nextThreadState.getEffectivePriority() > maxPriority)
					maxPriority = nextThreadState.getEffectivePriority();
			}
			while(next1.hasNext()){
				ThreadState nextThreadState = (ThreadState) next1.next();
				if(nextThreadState.getEffectivePriority()==maxPriority)
					maxThread = nextThreadState;
			}
			if(maxThread!=null)
				System.out.println("The next Thread Chosen to run is: " + maxThread.thread.getName()+" with effective of "+ maxThread.getEffectivePriority());
			return maxThread;
		}
		...
		public void print() {
			...
			while(waitPQueue.iterator().hasNext()){
				ThreadState next = (ThreadState) waitPQueue.iterator().next();
				System.out.println(next.thread.getName() +" with effective priority of " + next.getEffectivePriority());
			}
		...
		HashSet<ThreadState> waitPQueue ;
	...
	protected class ThreadState {
		...
		public int getEffectivePriority() {
			// implement me
			return Math.max(effective,priority);
		}
		...
		public void waitForAccess(PriorityQueue waitQueue) {
			...
			//It is now waiting for the resource that is owned by the queue owner
